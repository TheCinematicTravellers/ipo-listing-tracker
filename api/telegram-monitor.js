...
let cachedMaster=null,cachedMasterAt=0;
...
async function nfoQuote(a,token){await paceAngel(400);const r=await fetch(QUOTE_URL,{method:'POST',headers:angelHeaders(a.apiKey,a.jwt,a.mac),body:JSON.stringify({mode:'LTP',exchangeTokens:{NFO:[String(token)]}})}),d=await r.json();if(!r.ok||d?.status===false)throw Error(`Option LTP failed: ${d?.message||r.status}`);const q=d?.data?.fetched?.[0]||d?.data?.fetched?.[String(token)]||d?.data?.[0];const ltp=Number(q?.ltp);if(!Number.isFinite(ltp)||ltp<=0)throw Error('Invalid option LTP');return ltp}
async function placeTestOrder(a,underlying,direction,date){const all=await loadMaster(),now=new Date(),eligible=all.filter(x=>String(x.exch_seg).toUpperCase()==='NFO'&&String(x.instrumenttype).toUpperCase()==='OPTSTK'&&String(x.name).toUpperCase()===String(underlying).toUpperCase()&&['CE','PE'].includes(String(x.symbol).slice(-2)));
  const expiries=[...new Set(eligible.map(x=>x.expiry).filter(Boolean))].map(e=>({raw:e,date:expiryDate(e)})).filter(x=>x.date&&x.date>=new Date(Date.UTC(now.getUTCFullYear(),now.getUTCMonth(),now.getUTCDate()))).sort((a,b)=>a.date-b.date);
  if(!expiries.length)throw Error(`No active NFO stock-option expiry for ${underlying}`);
  const expiry=expiries[0].raw,opts=eligible.filter(x=>x.expiry===expiry),underlyingLtp=Number(underlyingLtpCache.get(underlying));
  if(!Number.isFinite(underlyingLtp))throw Error(`Missing underlying LTP for ${underlying}`);
  const strikes=[...new Set(opts.map(x=>optionStrike(x.strike)).filter(Number.isFinite))].sort((a,b)=>a-b);if(!strikes.length)throw Error(`No strikes for ${underlying}`);
  let atmIndex=0;for(let i=1;i<strikes.length;i++)if(Math.abs(strikes[i]-underlyingLtp)<Math.abs(strikes[atmIndex]-underlyingLtp))atmIndex=i;
  const targetIndex=direction==='LONG'?atmIndex-1:atmIndex+1;if(targetIndex<0||targetIndex>=strikes.length)throw Error(`ATM adjacent strike unavailable for ${underlying}`);
  const targetStrike=strikes[targetIndex],optType=direction==='LONG'?'CE':'PE',contract=opts.find(x=>String(x.symbol).endsWith(optType)&&Math.abs(optionStrike(x.strike)-targetStrike)<1e-9);
  if(!contract)throw Error(`Option contract not found for ${underlying} ${targetStrike} ${optType}`);
  const ltp=await nfoQuote(a,contract.token),tickRaw=Number(contract.tick_size),tick=Number.isFinite(tickRaw)&&tickRaw>0?tickRaw:0.05,rawBid=ltp*0.70,bid=Math.floor((rawBid+1e-9)/tick)*tick,price=Number(bid.toFixed(4)),qty=Math.max(1,Number(contract.lotsize)||1);
  await paceAngel(150);const payload={variety:'NORMAL',tradingsymbol:contract.symbol,symboltoken:String(contract.token),transactiontype:'BUY',exchange:'NFO',ordertype:'LIMIT',producttype:'NRML',duration:'DAY',price:String(price),quantity:String(qty),ordertag:'ORBTEST'};
  const r=await fetch(ORDER_URL,{method:'POST',headers:angelHeaders(a.apiKey,a.jwt,a.mac),body:JSON.stringify(payload)}),d=await r.json();if(!r.ok||!d?.status)throw Error(`Angel order failed: ${d?.message||d?.errorcode||r.status}`);
  return {underlying,direction,underlying_ltp:underlyingLtp,expiry,option:contract.symbol,option_token:String(contract.token),strike:targetStrike,option_type:optType,option_ltp:ltp,test_bid:price,quantity:qty,tick_size:tick,order_id:d?.data?.orderid||d?.data?.order_id||null};
}
const underlyingLtpCache=new Map();
async function testOrderForAlert(row,direction,date){const key=orderKey(date,row.symbol),claimed=await claimOrder(key);if(claimed!== 'OK')return {skipped:true,reason:'Order already attempted'};try{const a=await angelLogin();underlyingLtpCache.set(row.symbol,Number(row.cmp));const result=await placeTestOrder(a,row.symbol,direction,date);await setState(key,JSON.stringify({status:'PLACED',...result}));return result}catch(e){await setState(key,JSON.stringify({status:'FAILED',error:e.message||String(e)}));throw e}}
export default async function handler(req,res){
  ...
  try{
    ...
    for(const row of rows){
      ...
      let orderText='';
      if(status==='✅ Trade Active'){
        try{const result=await testOrderForAlert(row,row.direction,date);if(result.skipped)orderText='Order test already attempted';else{orders.push(result);orderText=`\n*TEST ORDER*\nBUY ${result.option}\nOption LTP: ${result.option_ltp}\nTest bid: ${result.test_bid}\nQty: ${result.quantity}\nOrder ID: ${result.order_id||'accepted'}`}}catch(e){orderText=`\n*TEST ORDER FAILED*\n${esc(e.message||String(e))}`}
      }
      const text=`*${esc(title)}*\n*${esc(row.symbol)}*\nStatus: ${esc(status)}\nChange: ${esc(row.change_pct+'%')}\nTime: ${esc(`${p.hour}:${p.minute} IST`)}${orderText}`;
