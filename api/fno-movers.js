...
async function login(){const apiKey=process.env.ANGEL_API_KEY,client=process.env.ANGEL_CLIENT_ID,pin=process.env.ANGEL_PIN,secret=process.env.ANGEL_TOTP_SECRET,mac=process.env.ANGEL_MAC_ADDRESS||'00:00:00:00:00:00';if(!apiKey||!client||!pin||!secret)throw Error('Missing Angel One environment variables');const r=await fetch(LOGIN_URL,{method:'POST',headers:headers(apiKey,null,mac),body:JSON.stringify({clientcode:client,password:pin,totp:totp(secret)})}),d=await r.json();if(!r.ok||!d?.data?.jwtToken)throw Error(`Angel login failed: ${d?.message||r.status}`);return {apiKey,jwt:d.data.jwtToken,mac}}
async function quote(a,tokens){const r=await fetch(QUOTE_URL,{method:'POST',headers:headers(a.apiKey,a.jwt,a.mac),body:JSON.stringify({mode:'FULL',exchangeTokens:{NSE:tokens}})}),d=await r.json();if(!r.ok||d?.status===false)throw Error(`Quote failed: ${d?.message||r.status}`);return d?.data?.fetched||[]}
async function candles(a,token,from,to,interval){const r=await fetch(CANDLE_URL,{method:'POST',headers:headers(a.apiKey,a.jwt,a.mac),body:JSON.stringify({exchange:'NSE',symboltoken:String(token),interval,fromdate:from,todate:to})}),d=await r.json();if(!r.ok||d?.status===false)throw Error(`Candle failed: ${d?.message||r.status}`);return d?.data||[]}
...
function stateFor(orbCandles,oneMin,direction){
  ...
  return entry===null?{status:'⏳ Pending',result:'⏳'}:{status:'✅ Trade Active',result:'⚖️'};
}
export default async function handler(req,res){res.setHeader('Access-Control-Allow-Origin','*');res.setHeader('Access-Control-Allow-Methods','GET,OPTIONS');res.setHeader('Access-Control-Allow-Headers','Content-Type');res.setHeader('Cache-Control','no-store,max-age=0');if(req.method==='OPTIONS')return res.status(204).end();if(req.method!=='GET')return res.status(405).json({error:'GET only'});try{const raw=await fetch('https://raw.githubusercontent.com/TheCinematicTravellers/ipo-listing-tracker/main/fno_universe.json',{cache:'no-store'}).then(r=>r.json()),symbols=Array.isArray(raw)?raw:JSON.parse(raw.content),m=await master(),matched=symbols.map(s=>({symbol:s,token:m.get(norm(s))})).filter(x=>x.token),missing=symbols.filter(s=>!m.get(norm(s))),a=await login(),rows=[];for(let i=0;i<matched.length;i+=50){const batch=matched.slice(i,i+50),f=await quote(a,batch.map(x=>String(x.token))),by=new Map(f.map(x=>[String(x.symbolToken),x]));for(const x of batch){const q=by.get(String(x.token));if(!q)continue;const open=num(q.open),high=num(q.high),low=num(q.low),cmp=num(q.ltp),close=num(q.close),volume=num(q.tradeVolume??q.volume);if([open,high,low,cmp,close].some(v=>v===null)||!close)continue;rows.push({symbol:x.symbol,token:x.token,change_pct:(cmp/close-1)*100,open,high,low,cmp,volume:volume??0})}}
