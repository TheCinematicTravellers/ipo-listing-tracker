...
TEST_SETUP_MINUTE = "12:50"
TEST_FREEZE_TIME = time(12, 51, 2)
...
def rank_top_movers(rows: list[dict[str, Any]], top_n: int = TOP_N):
    ...
    for row in rows:
        try:
            ...
            continue
        if ltp < 100.0 or close <= 0 or ltp > MAX_STOCK_PRICE:
            continue
        item = dict(row)
        item["ltp"] = ltp
        item["close"] = close
...
def main():
    ...
    if LATE_START_TEST:
        setup_minute = TEST_SETUP_MINUTE
        freeze_at = datetime.combine(now.date(), TEST_FREEZE_TIME, tzinfo=IST)
        if now >= freeze_at:
            print(f"[SAFE STOP] TEST setup window {setup_minute} already closed at {TEST_FREEZE_TIME.strftime('%H:%M:%S')} IST")
            return
    ...
    print(f"[READY] {len(states)} eligible setups. Live stock trigger monitoring active until 15:13 IST.")
    ...
    try:
        ...
        for symbol, state in states.items():
            ...
            if state.entered:
                send_exit(symbol, state, "TIME_EXIT_15_13", state.option_ltp)
            elif not state.trade_used:
                print(f"[EXPIRED] {symbol} {state.setup.side} | no option entry before 15:13")
        try:
            sws.close_connection()
        except Exception:
            pass
        ws_thread.join(timeout=3)
        print("[STOP] 15:13 IST | runner closed cleanly")
