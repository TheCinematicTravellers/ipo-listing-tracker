...
def test_policybazar_trading_day_is_tuesday_to_thursday_only():
    ...
    assert is_trading_day(date(2026, 9, 1)) is True   # Tuesday
    assert is_trading_day(date(2026, 9, 2)) is True   # Wednesday
    assert is_trading_day(date(2026, 9, 3)) is True   # Thursday
    assert is_trading_day(date(2026, 8, 31)) is False # Monday
    assert is_trading_day(date(2026, 9, 4)) is False  # Friday


def test_policybazar_breakout_window_is_before_10am():
    from policybazar_live_runner import breakout_allowed

    assert breakout_allowed(time(9, 16)) is True
