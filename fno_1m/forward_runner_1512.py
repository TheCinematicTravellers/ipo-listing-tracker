"""15:12 live forward test using locally collected 15:11 candle."""
...
SETUP_START=time(15,11); ACTIVATION=time(15,12)
...
def wait_until_setup():
    while datetime.now(IST).time()<SETUP_START:
        ...
        print(f"[WAIT] Live setup collection starts 15:11 IST | current={now:%H:%M:%S} | remaining={max(0,int((target-now).total_seconds()))}s")
        time_mod.sleep(min(10,max(1,(target-now).total_seconds())))
def collect(api,feed_token,master,symbols):
    token_map=runner.nse_tokens(master,symbols); c=MinuteCandleCollector("15:11")
    sws=SmartWebSocketV2(api.access_token,runner.API_KEY,runner.CLIENT_ID,feed_token)
    def on_open(ws):
        tokens=list(token_map.values())
        for i in range(0,len(tokens),50):
            sws.subscribe(f"setup_1512_{i//50}",runner.LTP,[{"exchangeType":runner.NSE,"tokens":tokens[i:i+50]}])
        print(f"[LIVE FEED] Subscribed {len(tokens)} NSE stocks for 15:11 candle")
    def on_data(ws,msg):
        try:
    ...
    print(f"[LOCAL CANDLE] 15:11 complete | stocks with ticks={len(c._bars)}")
    return c
def main():
    ...
    print(f"[OK] Real F&O universe: {len(runner.nse_tokens(master,symbols))}"); print("[TEST] Setup=15:11-15:12 | Activation=15:12 | Candle source=LIVE WEBSOCKET | Historical candle API=DISABLED"); print("[OK] AlgoTest forward-only webhook configured")
    collector=collect(api,feed,master,symbols)
    def local_candle(_api,token,_day):
        v=collector.candle(token)
        if v is None: raise RuntimeError(f"No live 15:11 candle collected for token {token}")
        return v
    runner.candle_0915=local_candle; runner.run_historical_catchup=lambda *_a,**_k: print("[FORWARD 15:12] Historical catch-up disabled"); runner.ENTRY_TIME=ACTIVATION
