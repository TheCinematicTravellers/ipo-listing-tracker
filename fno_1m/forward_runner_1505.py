...
SETUP_START=time(15,4); ACTIVATION=time(15,5)
...
def wait_until_setup():
    while datetime.now(IST).time()<SETUP_START:
        ...
        print(f"[WAIT] Live setup collection starts 15:04 IST | current={now:%H:%M:%S} | remaining={max(0,int((target-now).total_seconds()))}s")
        time_mod.sleep(min(10,max(1,(target-now).total_seconds())))
def collect(api,feed_token,master,symbols):
    token_map=runner.nse_tokens(master,symbols); c=MinuteCandleCollector("15:04")
    ...
    def on_open(ws):
        ...
        print(f"[LIVE FEED] Subscribed {len(tokens)} NSE stocks for 15:04 candle")
    def on_data(ws,msg):
        try:
    ...
    print(f"[LOCAL CANDLE] 15:04 complete | stocks with ticks={len(c._bars)}")
    return c
def main():
    ...
    print(f"[OK] Real F&O universe: {len(runner.nse_tokens(master,symbols))}"); print("[TEST] Setup=15:04-15:05 | Activation=15:05 | Candle source=LIVE WEBSOCKET | Historical candle API=DISABLED"); print("[OK] AlgoTest forward-only webhook configured")
    collector=collect(api,feed,master,symbols)
    def local_candle(_api,token,_day):
        v=collector.candle(token)
        if v is None: raise RuntimeError(f"No live 15:04 candle collected for token {token}")
        return v
