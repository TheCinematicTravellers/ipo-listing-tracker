...
def test_dryrun_core_path():
    ...
    for j, price in enumerate((100.0, 101.0, 99.0, 101.0)):
        collector.on_ltp("1", price, base + timedelta(seconds=j * 15))
    assert collector.candle("1") == (100.0, 101.0, 99.0, 101.0)

    setup = make_setup("TEST", 100.0, 101.0, 99.0, 101.0, "LONG")
    assert setup is not None
    assert setup.entry_level == 101.0
    assert setup.stock_sl == 99.0
