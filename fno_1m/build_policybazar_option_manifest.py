...
def historical_monthly_expiries(year_months: list[tuple[int, int]], trading_dates: list[date]) -> list[date]:
    """Resolve historical NSE stock-option monthly expiries.

    Contracts expiring on/before 2025-08-31 use the legacy Thursday expiry.
    Contracts expiring from 2025-09-01 use the last Tuesday of the month; if
    that Tuesday is not present in the historical trading calendar, walk back
    to the previous available trading day. The final month does not need to
    have an expiry candle in the stock dataset.
    """
    trading = set(trading_dates)
    out: list[date] = []
    for year, month in sorted(set(year_months)):
        if (year, month) == (2025, 8):
            candidate = date(2025, 8, 28)
            if candidate not in trading:
                raise ValueError("Trading calendar missing 2025-08-28, required for August 2025 expiry")
            out.append(candidate)
            continue
        month_start = date(year, month, 1)
        last_day = _last_calendar_day(year, month)
        # Last Tuesday.
        candidate = last_day - timedelta(days=(last_day.weekday() - 1) % 7)
        # A holiday moves expiry to the previous trading day.
        while candidate >= month_start and candidate not in trading:
...
def build(locked_csv: Path, master_path: Path, output: Path, calendar_csv: Path | None = None) -> int:
    ...
    signal_dates = [date.fromisoformat(sig["date"]) for sig in signals]
    if not signal_dates:
        raise RuntimeError("No signals in locked S1 CSV")
    months = {(d.year, d.month) for d in signal_dates}
    historical_expiries = historical_monthly_expiries(sorted(months), trading_dates)
    ...
    for sig in signals:
        ...
        if not future:
            # Add the next calendar month if the signal occurs after the
            # current month's expiry. This is needed for signals late in the
            # month, while keeping the expiry itself independent of stock data.
            y, m = day.year, day.month
            if m == 12:
                y, m = y + 1, 1
            else:
                m += 1
            extra = historical_monthly_expiries([(y, m)], trading_dates)
            historical_expiries.extend(extra)
            future = [e for e in historical_expiries if e >= day]
        if not future:
    ...
    if missing_master_expiries:
        ...
        raise RuntimeError(
            "Historical expiry dates resolved correctly, but the supplied current Angel master "
            f"has no POLICYBZR contracts for: {detail}. Do not substitute a current expiry; "
            "a historical contract-master snapshot is required."
        )
...
if __name__ == "__main__":
    ...
    p.add_argument("--calendar-csv", help="Raw 5m stock CSV used only for trading-date/holiday calendar")
