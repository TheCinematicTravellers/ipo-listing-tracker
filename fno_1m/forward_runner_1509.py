...
SETUP_START=time(15,8); ACTIVATION=time(15,9)
...
def wait_until_setup():
    while datetime.now(IST).time()<SETUP_START:
        ...
        print(f"[WAIT] Live setup collection starts 15:08 IST | current={now:%H:%M:%S} | remaining={max(0,int((target-now).total_seconds()))}s")
        time_mod.sleep(min(10,max(1,(target-now).total_seconds())))
def collect(api,feed_token,master,symbols):
    token_map=runner.nse_tokens(master,symbols); c=MinuteCandleCollector("15:08")
