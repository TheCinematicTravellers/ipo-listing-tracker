...
def main():
    ...
    if a.mode in {"manifest","all"}:
        run([sys.executable,"build_policybazar_option_manifest.py","--stock-csv",a.stock_csv,"--master",a.master,"--output",str(manifest)])
    ...
    if a.mode in {"backtest","all"}:
        run([sys.executable,"backtest_policybazar_options.py","--manifest",str(manifest),"--stock-csv",a.stock_csv,"--raw",str(root/"raw"),"--output",str(trades),"--option-stop-pct",str(a.option_stop_pct)])
